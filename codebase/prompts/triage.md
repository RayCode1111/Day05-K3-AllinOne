<!-- version: v4 · chốt 2026-07-30 · đổi prompt phải tăng version và ghi vào spec.md §9 -->
...
8. **Tài liệu kèm việc phải làm thì tính theo việc, không tính theo tài liệu.** Một tin vừa có tài liệu/link mới vừa dặn phải đọc/tải/dùng/nộp trong ngày khớp cùng lúc hai dòng trong bảng trên — khi đó xếp theo **mốc thời gian của việc** (`NGAY` hoặc `HOM_NAY`), không xếp `GHI_NHO`. Dòng `GHI_NHO` chỉ dành cho tài liệu **không kèm** việc phải làm trong ngày.
