...
Data pack Discord CÓ tồn tại (`data/discord-pack`, ngoài repo theo luật bảo mật)
nhưng là text thô: không có `ts` / `tac_gia` / `vai` / `tra_loi_cho` cho từng tin,
mà `ts` là gốc của mọi ngưỡng mức và `vai` là đầu vào của guard
`nguon_khong_chinh_thuc`. Lý do đầy đủ vì sao không dùng: spec.md §7.
"""
